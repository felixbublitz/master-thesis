...
class Server{
    ...
    constructor(){
        ...
        this.app.use('/extractor', express.static(__dirname + '/extractor'));
        this.app.use('/library', express.static(__dirname + '/library'));
        this.app.use('/assets', express.static(__dirname + '/assets'));

        this.app.listen(config.APPLICATION_PORT, () => {})

        this.server = new WebSocketServer({ port : config.SERVER_PORT });
